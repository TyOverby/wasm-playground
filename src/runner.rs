...
fn main() {
    ...
    // This test shows how to implement native module
    // https://github.com/NikVolf/parity-wasm/blob/master/src/interpreter/tests/basics.rs#L197

    let mut externals = Host { counter: 0 };
    let main = ModuleInstance::new(
        &module,
        &ImportsBuilder::new().with_resolver("host", &externals),
    ...
    println!(
        "Result: {:?}",
        main.invoke_export("_call", &[RuntimeValue::I32(argument)], &mut externals)
    );

    let memory = main.export_by_name("mem").unwrap();
    let memory = memory.as_memory().unwrap();
    let values = memory.get(0, 4).unwrap();
    println!("{:?}", values);
}
